...
//  ZoomVideoSample
//
//  Created by Zoom Communications on 2019/6/5.
...
typedef NS_ENUM(NSInteger, ZoomVideoRendererType) {
    ...
    ZoomVideoRendererType_Zoom_OpenGLES,
    ZoomVideoRendererType_Zoom_Metal
};

@interface CreateViewController () <UITextFieldDelegate, ZoomVideoSDKDelegate>
@property (nonatomic, strong) UILabel       *sessionNameLabel;
@property (nonatomic, strong) UITextField   *sessionNameTF;
@property (nonatomic, strong) UILabel       *steamingJoinLabel;
@property (nonatomic, strong) UITextField       *steamingJoinTF;
...
@property (nonatomic, assign) ZoomVideoRendererType    rendererType;
...
- (void)viewDidLoad {
    ...
    } else if (self.type == ZoomVideoCreateJoinType_Join) {
...
- (void)initSubView {
    ...
    self.tokenTF.clearButtonMode = UITextFieldViewModeWhileEditing;
    ...
    self.steamingJoinLabel.text = @"Steaming Join ChannelID";
    ...
    self.steamingJoinTF.clearButtonMode = UITextFieldViewModeWhileEditing;
    self.steamingJoinTF.textColor = RGBCOLOR(0x23, 0x23, 0x33);;
    self.steamingJoinTF.font = [UIFont boldSystemFontOfSize:15.0];
    self.steamingJoinTF.autocapitalizationType = UITextAutocapitalizationTypeNone;
    self.steamingJoinTF.autocorrectionType = UITextAutocorrectionTypeNo;
...
- (void)onCleanupResult:(ZoomVideoSDKError)errorType {
    NSLog(@"---- %s ------  %d ",__func__,errorType);
}
...
{
    // Only convert sessionNameTF to lowercase, keep steamingJoinTF case-sensitive
...
#else
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Renderer" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Zoom Canvas" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        self.rendererType = ZoomVideoRendererType_Zoom_Canvas;
        self.rendererResultLabel.text = @"Zoom renderer";
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Metal renderer" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        self.rendererType = ZoomVideoRendererType_Zoom_Metal;
    ...
    selectView.selectRendererOnClickBlock = ^(NSInteger selectIndex) {
        switch (selectIndex) {
            ...
            case 1:
                ...
                self.rendererType = ZoomVideoRendererType_Zoom_OpenGLES;
                break;
            case 2:
                self.rendererResultLabel.text = @"Metal renderer";
                self.rendererType = ZoomVideoRendererType_Zoom_Metal;
...
//    self.yuvAdapter = [[SendYUVAdapter alloc] init];
    ...
    ZoomVideoSDKSessionContext *sessionContext = [ZoomVideoSDKSessionContext new];
...
{
    NSLog(@"onMicSpeakerVolumeChanged========%@,%@",@(micVolume),@(speakerVolume));
}

- (void)onTestMicStatusChanged:(ZoomVideoSDKTestMicStatus)status;
{
    NSLog(@"onTestMicStatusChanged========%@",@(status));
}

...
#define kmoreTag        11004

#define video_reaction_size 25.0f
#define video_more_size 25.0f
...
- (void)addThumberViewItem:(ViewItem *)item; {
    ...
    [clickMore addTarget:self action:@selector(onVideoMoreClicked:) forControlEvents:UIControlEventTouchUpInside];
    clickMore.tag = kmoreTag;
...
- (void)onVideoMoreClicked:(UIButton *)sender
{
    ...
    if (_isCameraControl == NO) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Request Camera Control"
    ...
    if ([[[[ZoomVideoSDK shareInstance] getSession] getMySelf] isHost] || [[[[ZoomVideoSDK shareInstance] getSession] getMySelf] isManager]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Remove"
            ...
            for (ViewItem *item in self.viewArray) {
                if (item.isPin) {
                    BOOL ret = [[[ZoomVideoSDK shareInstance] getUserHelper] removeUser:item.user];
